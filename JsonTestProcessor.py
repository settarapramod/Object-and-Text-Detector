...
def create_datasets_from_json(json_data, structure, id_config):
    ...
    def process_node(node, parent_id, parent_key):
        # If parent_key is None, we are at the root level
        ...
        for key, value in node.items():
            ...
            print(f"Processing table: {table_name}, Parent key: {parent_key}")  # Debug print

            # If the table name is part of the structure, process it
            if table_name in structure:
                columns = structure[table_name]
                rows = []
                print(f"Columns for {table_name}: {columns}")  # Debug print
                ...
                if isinstance(value, list):
                    for item in value:
                        ...
                        print(f"Row for {table_name}: {row}")  # Debug print
                        # Add ID column if configured
                        if table_name in id_config:
                            id_info = id_config[table_name]
                            id_column = id_info["id_column"]
                            ...
                            # If not in immediate parent, search the entire JSON
                            else:
